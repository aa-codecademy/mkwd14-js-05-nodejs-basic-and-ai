...
async function loadStandings() {
	...
	try {
		...
		if (!teams.length) {
			el.innerHTML = '<p class="loading">No teams yet.</p>';
			return;
		}
...
async function createTeam() {
	...
	if (!name || !country) {
		showAlert('Name and country are required.', true);
		return;
	}
...
async function loadTeamSelects() {
	try {
		...
		const options = teams
			...
			.join('');
		document.getElementById('home-team-select').innerHTML =
			`<option value="">Home team…</option>${options}`;
		document.getElementById('away-team-select').innerHTML =
			`<option value="">Away team…</option>${options}`;
	} catch (_) {}
}
...
async function loadMatches() {
	...
	try {
		...
		if (!matches.length) {
			el.innerHTML = '<p class="loading">No matches found.</p>';
			return;
		}
...
function renderMatchCard(m) {
	...
	if (m.status === 'scheduled') {
		actions = `
          <button class="btn btn-green" onclick="doStart('${m._id}')">▶ Start</button>
          <button class="btn btn-yellow" onclick="doPostpone('${m._id}')">⏸ Postpone</button>`;
	}
	if (m.status === 'live') {
		actions = `
          <button class="btn btn-green" onclick="toggleGoalForm('${m._id}')">⚽ Goal</button>
          <button class="btn btn-red" onclick="doFinish('${m._id}')">⏹ Full Time</button>`;
	}
	if (m.status === 'postponed') {
		actions = `<button class="btn btn-green" onclick="doStart('${m._id}')">▶ Start anyway</button>`;
	}

	const goalForm =
		m.status === 'live'
			? `
      <div class="goal-form" id="goal-form-${m._id}">
        <div class="form-row">
          <select id="goal-team-${m._id}">
            <option value="${m.homeTeamId}">${m.homeTeamName}</option>
            <option value="${m.awayTeamId}">${m.awayTeamName}</option>
          </select>
          <input type="text" id="goal-scorer-${m._id}" placeholder="Scorer name" />
          <input type="number" id="goal-minute-${m._id}" placeholder="Minute" min="1" max="120" />
          <button class="btn btn-green" onclick="doGoal('${m._id}')">Confirm</button>
          <button class="btn btn-gray" onclick="toggleGoalForm('${m._id}')">Cancel</button>
        </div>
      </div>`
			: '';

	return `
      <div class="match-card ${m.status}" id="match-${m._id}">
...
async function scheduleMatch() {
	...
	if (!homeTeamId || !awayTeamId) {
		showAlert('Select both teams.', true);
		return;
	}
	if (homeTeamId === awayTeamId) {
		showAlert('A team cannot play itself.', true);
		return;
	}
...
async function doFinish(id) {
	try {
		...
		showAlert(
			`Full time! ${m.homeTeamName} ${m.homeScore}–${m.awayScore} ${m.awayTeamName}`,
		);
		loadMatches();
		loadStandings();
	} catch (e) {
...
async function doGoal(matchId) {
	...
	try {
		...
		showAlert(
			`⚽ Goal! ${m.homeTeamName} ${m.homeScore}–${m.awayScore} ${m.awayTeamName}`,
		);
		loadMatches();
	} catch (e) {
		showAlert(e.message, true);
